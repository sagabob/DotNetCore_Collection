...
public class ArticleService:IArticleService
{
    ...
    public async Task<IResult> UpdateArticle(int id, ArticleRequest article)
    {
        ...
        if (article.Title != null) articleToUpdate.Title = article.Title;

        if (article.Content != null) articleToUpdate.Content = article.Content;

        if (article.PublishedAt != null) articleToUpdate.PublishedAt = article.PublishedAt;
